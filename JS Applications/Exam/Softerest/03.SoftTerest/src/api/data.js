...
export async function getAllItems(page = 1) {
    return await api.get(host + `/data/ideas?offset=${(page-1)*3}&pageSize=3`);
}

export async function getCollectionSize() {
    return await api.get(host + '/data/ideas?count')
}
...
export async function getMyItems() {
    ...
    return await api.get(host + `/data/ideas?where=_ownerId%3D%22${userId}%22`);
