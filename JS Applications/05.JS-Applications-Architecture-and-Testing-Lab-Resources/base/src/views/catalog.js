...
function createPager(page, pages, goTo) {
    ...
    if (page > 1) {
        result.appendChild(e('a', { ckassName: 'pager', href: '/catalog', onClick: (ev) => changePage(ev, page - 1) }, 'Prev >'));
    }
    if (page < pages) {
        result.appendChild(e('a', { ckassName: 'pager', href: '/catalog', onClick: (ev) => changePage(ev, page + 1) }, 'Next >'));
    }
    return result;
